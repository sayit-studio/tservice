...
- 預約資訊查詢：由 LINE OA 回覆「您已預約成功，日期與時間如下」
- 取消預約：民眾取消後，系統主動通知服務同仁
- 所有表單結果送到 n8n webhook，再由 n8n 呼叫 LINE Messaging API 回覆民眾或通知團隊
...
正式環境建議把 `teamLineTarget` 固定在 n8n 端，不要完全信任前端傳入值。
...
- `legal-consultation/n8n-workflow.md`：n8n 流程與 LINE Flex Message 範本
