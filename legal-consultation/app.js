(function () {
  ...
  function validatePayload(payload, mode) {
    ...
    if (mode === "query" && !payload.appointmentId && !payload.phoneLast3) {
      return "請至少填寫預約編號或聯絡電話末三碼。";
    }
    return "";
  }
  ...
  function buildPayload(mode, form) {
    return {
      ...
      submittedAt: new Date().toISOString(),
      line: {
        userId: state.profile && state.profile.userId ? state.profile.userId : "",
        displayName: state.profile && state.profile.displayName ? state.profile.displayName : "",
        pictureUrl: state.profile && state.profile.pictureUrl ? state.profile.pictureUrl : ""
      },
  ...
  function closeLiffWindow() {
    ...
    showNotice("此頁面已完成送出，請回到 LINE 查看通知。", "success");
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const fallbackText = "請回到 LINE 查看系統通知。";
